import { useState, useEffect, useRef } from 'react';
import { formatUnits } from 'viem';
import { config } from '../config/wagmi';

export interface BalanceResult {
  chainId: number;
  balance: bigint | null;
  symbol: string;
  decimals: number;
  isLoading: boolean;
  error: Error | null;
}

interface UseStaggeredBalancesOptions {
  /** Delay between each chain's balance fetch in ms (default: 200) */
  staggerDelay?: number;
  /** Whether to enable fetching (default: true) */
  enabled?: boolean;
}

/**
 * Hook to fetch balances for multiple chains with staggered requests.
 * Prevents rate limiting by adding a delay between each chain's balance fetch.
 */
export function useStaggeredBalances(
  address: `0x${string}` | undefined,
  chainIds: number[],
  options: UseStaggeredBalancesOptions = {}
): BalanceResult[] {
  const { staggerDelay = 200, enabled = true } = options;
  const [balances, setBalances] = useState<Map<number, BalanceResult>>(new Map());
  const abortControllerRef = useRef<AbortController | null>(null);

  useEffect(() => {
    if (!address || !enabled || chainIds.length === 0) {
      setBalances(new Map());
      return;
    }

    // Cancel any ongoing fetches
    abortControllerRef.current?.abort();
    abortControllerRef.current = new AbortController();
    const signal = abortControllerRef.current.signal;

    // Initialize all chains as loading
    const initialBalances = new Map<number, BalanceResult>();
    chainIds.forEach(chainId => {
      const chain = config.chains.find(c => c.id === chainId);
      initialBalances.set(chainId, {
        chainId,
        balance: null,
        symbol: chain?.nativeCurrency.symbol || 'ETH',
        decimals: chain?.nativeCurrency.decimals || 18,
        isLoading: true,
        error: null,
      });
    });
    setBalances(initialBalances);

    // Fetch balances with staggered delays
    const fetchBalances = async () => {
      for (let i = 0; i < chainIds.length; i++) {
        if (signal.aborted) return;

        const chainId = chainIds[i];
        const chain = config.chains.find(c => c.id === chainId);

        // Add stagger delay (skip first)
        if (i > 0) {
          await new Promise(resolve => setTimeout(resolve, staggerDelay));
        }

        if (signal.aborted) return;

        try {
          // Create a client for this specific chain
          const { createPublicClient, http } = await import('viem');
          const { RPC_ENDPOINTS } = await import('../config/rpc');

          const rpcUrls = RPC_ENDPOINTS[chainId] || [];
          const rpcUrl = rpcUrls[0]; // Use first RPC for balance check

          if (!chain || !rpcUrl) {
            throw new Error(`No RPC configured for chain ${chainId}`);
          }

          const client = createPublicClient({
            chain,
            transport: http(rpcUrl, { timeout: 10_000 }),
          });

          const balance = await client.getBalance({ address });

          if (signal.aborted) return;

          setBalances(prev => {
            const updated = new Map(prev);
            updated.set(chainId, {
              chainId,
              balance,
              symbol: chain.nativeCurrency.symbol,
              decimals: chain.nativeCurrency.decimals,
              isLoading: false,
              error: null,
            });
            return updated;
          });
        } catch (error) {
          if (signal.aborted) return;

          setBalances(prev => {
            const updated = new Map(prev);
            const existing = prev.get(chainId);
            updated.set(chainId, {
              chainId,
              balance: null,
              symbol: existing?.symbol || 'ETH',
              decimals: existing?.decimals || 18,
              isLoading: false,
              error: error instanceof Error ? error : new Error('Failed to fetch balance'),
            });
            return updated;
          });
        }
      }
    };

    fetchBalances();

    return () => {
      abortControllerRef.current?.abort();
    };
  }, [address, chainIds.join(','), staggerDelay, enabled]);

  // Convert map to array in the same order as chainIds
  return chainIds.map(chainId => {
    const result = balances.get(chainId);
    if (result) return result;

    const chain = config.chains.find(c => c.id === chainId);
    return {
      chainId,
      balance: null,
      symbol: chain?.nativeCurrency.symbol || 'ETH',
      decimals: chain?.nativeCurrency.decimals || 18,
      isLoading: true,
      error: null,
    };
  });
}

/**
 * Format balance for display
 */
export function formatBalance(balance: bigint | null, decimals: number): string | null {
  if (balance === null) return null;
  return parseFloat(formatUnits(balance, decimals)).toFixed(4);
}

/**
 * Check if balance is sufficient (> 0.01)
 */
export function hasEnoughBalance(balance: bigint | null, decimals: number): boolean {
  if (balance === null) return false;
  return parseFloat(formatUnits(balance, decimals)) > 0.01;
}
